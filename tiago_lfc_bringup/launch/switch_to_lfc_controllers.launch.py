...
def generate_launch_description():
    # Arguments
    pkg_arg = DeclareLaunchArgument("pkg", default_value="tiago_lfc_bringup")
    lfc_yaml_arg = DeclareLaunchArgument(
        "lfc_yaml", default_value="config/linear_feedback_controller_params.yaml"
    )
    jse_yaml_arg = DeclareLaunchArgument(
        "jse_yaml", default_value="config/joint_state_estimator_params.yaml"
    )
    pc_yaml_arg = DeclareLaunchArgument(
        "pc_yaml", default_value="config/dummy_controllers.yaml"
    )

    pkg = LaunchConfiguration("pkg")
    lfcP = PathJoinSubstitution(
        [FindPackageShare(pkg), LaunchConfiguration("lfc_yaml")]
    )
    jseP = PathJoinSubstitution(
        [FindPackageShare(pkg), LaunchConfiguration("jse_yaml")]
    )
    ...
    chain_spawn_joint_state_estimator = RegisterEventHandler(
        OnProcessExit(
            target_action=spawn_linear_feedback_controller,
            on_exit=[spawn_joint_state_estimator],
        )
    )
    chain_spawn_passthrough_controller = RegisterEventHandler(
        OnProcessExit(
            target_action=spawn_joint_state_estimator,
            on_exit=[spawn_passthrough_controller],
        )
    )
    chain_switch_1 = RegisterEventHandler(
        OnProcessExit(
            target_action=spawn_passthrough_controller, on_exit=[deactivate_arm]
        )
    )
